...
class MainActivity : ComponentActivity(), Shizuku.OnRequestPermissionResultListener {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_CODE_GSI && resultCode == RESULT_OK) {
            data?.data?.let { uri: Uri ->
                // Copy selected GSI to cache
                val inputStream = contentResolver.openInputStream(uri)
                val gsiFile = File(cacheDir, "selected_gsi.img")
                FileOutputStream(gsiFile).use { out ->
                    inputStream?.copyTo(out)
                }
                // Call privileged service to install GSI dynamic system
                val svcIntent = Intent(this, PrivilegedService::class.java).apply {
                    action = "android.os.dynamicsystem.INSTALL_DYNAMIC_SYSTEM"
                    putExtra("dynamic_system_image", gsiFile.absolutePath)
                }
